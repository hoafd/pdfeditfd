"""
OCR Engine module using Tesseract OCR (CPU – basic) and PaddleOCR (GPU/CPU – best).
Provides text recognition, bounding box detection, line detection,
and image preprocessing for improved OCR accuracy.

Engine selection:
  - 'tesseract' : always available, CPU-only, portable bundled binary
  - 'paddleocr' : best accuracy; auto-uses GPU when CUDA is available,
                  falls back to CPU automatically at runtime.
Models are downloaded on-demand by PaddleOCR to the default cache dir.
"""
...
class OCREngine:
    ...
    def __init__(self):
        ...
        # Engine selection: 'tesseract' (CPU basic) or 'paddleocr' (GPU/CPU best)
        self.active_engine = "paddleocr" if PaddleOCR is not None else "tesseract"
        self.paddleocr_reader = None

    def _get_paddle_reader(self):
        if self.active_engine == "paddleocr" and self.paddleocr_reader is None:
            self.set_engine("paddleocr")
        return self.paddleocr_reader

    def set_engine(self, engine_name):
        """Switch OCR engine: 'tesseract' (CPU, basic) or 'paddleocr' (GPU→CPU, best)."""
        if engine_name == "paddleocr":
            if PaddleOCR is None:
                raise ImportError("PaddleOCR is not installed. Run setup to install it.")
            if self.paddleocr_reader is None:
                ...
                # Ép PaddleOCR tải model vào thư mục tools thay vì C:\Users\...
                paddle_models_dir = get_tools_dir() / "paddleocr"
                paddle_models_dir.mkdir(parents=True, exist_ok=True)
                
                old_userprofile = os.environ.get('USERPROFILE')
                old_home = os.environ.get('HOME')
                os.environ['USERPROFILE'] = str(paddle_models_dir)
                os.environ['HOME'] = str(paddle_models_dir)

                det_dir = str(paddle_models_dir / "whl" / "det" / p_lang)
                rec_dir = str(paddle_models_dir / "whl" / "rec" / p_lang)
                cls_dir = str(paddle_models_dir / "whl" / "cls")
                
                # Check if specific en rec model exists since rec_lang could be 'latin'
                if not os.path.exists(rec_dir) and p_lang == "en":
                    rec_dir = str(paddle_models_dir / "whl" / "rec" / "latin")

                # Try GPU first; auto-fallback to CPU on failure
                try:
                    # Force PaddleOCR to use the local tools/paddleocr directory
                    os.environ["PADDLE_OCR_BASE_DIR"] = str(paddle_models_dir)

                    self.paddleocr_reader = PaddleOCR(
                        use_angle_cls=True, lang=p_lang,
                    ...
                    # Warm-up probe
                    self.paddleocr_reader.ocr(
                        np.zeros((10, 10, 3), dtype=np.uint8), cls=False
                    )
                    ...
                    # Suggest cuDNN if NVIDIA GPU is present
                    try:
                        subprocess.run(
                            ["nvidia-smi"],
                        ...
                        if "cudnn" in str(gpu_err).lower():
                            try:
                                import tkinter.messagebox as _mb
                                _mb.showinfo(
                                    "Gợi ý tăng tốc AI",
                                    "Phát hiện Card Đồ Họa NVIDIA nhưng thiếu driver cuDNN.\n\n"
                                    "Ứng dụng đã tự động dùng CPU. Để AI chạy siêu tốc, "
                    ...
                    # Restore môi trường
                    if old_userprofile: os.environ['USERPROFILE'] = old_userprofile
                    if old_home: os.environ['HOME'] = old_home
            self.active_engine = "paddleocr"
            logger.info("OCR Engine: PaddleOCR (GPU/CPU).")
        else:
            self.active_engine = "tesseract"
            logger.info("OCR Engine: Tesseract (CPU).")
    ...
    def get_ocr_info(self):
        ...
        return {
            ...
            "has_tesseract": self.is_available() or get_tesseract_path().exists(),
        }
    ...
    def image_to_string(self, image, lang=None, preprocess=True):
        ...
        if self.active_engine == "paddleocr" and self._get_paddle_reader() is not None:
            if isinstance(img, Image.Image):
                cv_img = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
            else:
                cv_img = img
            results = self._get_paddle_reader().ocr(cv_img, cls=True)
    ...
    def find_text_position(self, image, search_text, lang=None):
        ...
        if self.active_engine == "paddleocr" and self._get_paddle_reader() is not None:
            ...
            read_results = self._get_paddle_reader().ocr(cv_img, cls=True)
