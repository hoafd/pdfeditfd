...
def get_tools_dir():
    ...
    if _is_frozen():
        # Prefer tools folder placed directly next to the EXE
        external_tools = Path(sys.executable).parent / "tools"
        if external_tools.exists():
            return external_tools
        # Fallback if bundled inside _internal
...
def get_tesseract_path():
    ...
    Only checks the internal/bundled tools directory as requested.
    """
...
def get_tessdata_dir():
    """Get the tessdata directory for the active Tesseract installation."""
    bundled_tessdata = get_tools_dir() / "Tesseract-OCR" / "tessdata"
    
    # 1. Prefer our bundled tools/Tesseract-OCR/tessdata if it has language files (e.g. downloaded by setup.ps1)
    if bundled_tessdata.exists() and any(bundled_tessdata.glob("*.traineddata")):
        return bundled_tessdata
        
    # 2. Fallback to tessdata next to the executable
    tess_exe = get_tesseract_path()
