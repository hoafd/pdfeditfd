...
class PDFEditorApp:
    ...
    def show_ocr_info(self):
        """Show information about available OCR engines."""
        from src.ocr_engine import get_ocr_engine
        engine = get_ocr_engine()
        info = engine.get_ocr_info()
        has_paddle = info.get('has_paddleocr', False)
        
        dialog = tk.Toplevel(self.root)
        dialog.title("OCR & System Status")
        dialog.geometry("600x520")
        dialog.configure(bg=COLORS["bg_dark"])
        dialog.transient(self.root)
        dialog.grab_set()
        source_label = "Hệ thống" if "Program Files" in str(info.get('path', '')) else "Nội bộ (tools/)"
        if not info.get('has_tesseract'):
            source_label = "Không tìm thấy"

        gpu_status = "Đang kiểm tra..."
        try:
            import subprocess
            res = subprocess.run(["nvidia-smi"], capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            if res.returncode == 0:
                gpu_status = "✅ Có GPU NVIDIA (Hỗ trợ tăng tốc OCR)"
            else:
                gpu_status = "❌ Không có GPU NVIDIA (Chỉ dùng CPU)"
        except Exception:
            gpu_status = "❌ Không có GPU NVIDIA (Chỉ dùng CPU)"

        from src.utils import _is_frozen
        
        is_compiled = _is_frozen()
        
        if is_compiled:
            install_help_text = """LƯU Ý DÀNH CHO BẢN CÀI ĐẶT (EXE):
Bạn đang chạy phiên bản đã được biên dịch (.exe).
• Để dùng Tesseract OCR: Vui lòng chép thư mục 'tools' 
  để song song với file .exe này.
• PaddleOCR: Chỉ có thể sử dụng nếu đã được đóng gói cùng 
  với file .exe trong quá trình build."""
        else:
            install_help_text = """Nếu bạn muốn cài đặt hoặc cập nhật OCR, hãy nhấn nút bên dưới."""

        content = f"""═══════════════════════════════════════════
   TRẠNG THÁI OCR HIỆN TẠI
═══════════════════════════════════════════

  Tesseract OCR: {"✅ Sẵn sàng" if info.get('has_tesseract') else "❌ Chưa cài đặt"}
    - Nguồn: {source_label}
    - Đường dẫn: {info.get('path', 'N/A')}
    
  PaddleOCR:     {"✅ Sẵn sàng" if has_paddle else "❌ Chưa cài đặt"}
    - Trạng thái GPU: {gpu_status}

═══════════════════════════════════════════
   CÀI ĐẶT / NÂNG CẤP OCR
═══════════════════════════════════════════

• Tesseract: OCR mặc định, nhẹ, độ chính xác tốt.
• PaddleOCR: OCR tốc độ cao, nhận diện Tiếng Việt xuất sắc, dùng GPU.
...
"""
        ...
        if not is_compiled:
            def run_setup():
                import subprocess
                import os
                script = os.path.join(os.getcwd(), "setup.ps1")
                subprocess.Popen(["powershell", "-ExecutionPolicy", "Bypass", "-File", script],
                                 creationflags=subprocess.CREATE_NEW_CONSOLE)
                messagebox.showinfo("Cài đặt OCR", "Đã mở cửa sổ cài đặt Tesseract. Vui lòng đợi cửa sổ đó chạy xong và KHỞI ĐỘNG LẠI ứng dụng.", parent=dialog)
                dialog.destroy()
            ...
            def install_paddle():
                ...
                messagebox.showinfo(
                    "Cài đặt PaddleOCR", 
                    "Ứng dụng sẽ tự động đóng lại để cài đặt (nhằm giải phóng thư viện đang chạy).\n\n"
                    "Quá trình tải sẽ mất vài phút tùy mạng. Vui lòng đợi cửa sổ màu đen chạy xong rồi tự KHỞI ĐỘNG LẠI ứng dụng.", 
                    parent=dialog
                )
                
                # Run pip install in a visible command window with a 3s delay so this app can exit first
                cmd = f'start cmd.exe /k "cd /d {os.getcwd()} && .\\venv\\Scripts\\activate && echo Dang doi ung dung dong lai (3 giay)... && timeout /t 3 /nobreak >nul && echo Dang tai PaddleOCR va PaddlePaddle (dung luong ~1GB, vui long cho vai phut)... && pip install \\"paddlepaddle<3.0.0\\" \\"paddleocr>=2.8.1,<3.0.0\\" && echo. && echo Cai dat thanh cong! Vui long dong cua so nay va khoi dong lai app. && pause"'
                os.system(cmd)
            ...
            tk.Button(btn_frame, text="⚙️ Cài đặt PaddleOCR", font=("Segoe UI", 11, "bold"),
                      bg=COLORS["success"], fg=COLORS["bg_dark"],
                      activebackground="#8bd588", relief=tk.FLAT, padx=15, pady=8,
                      command=install_paddle).pack(side=tk.LEFT, padx=5)

            tk.Button(btn_frame, text="🔧 Sửa lỗi Tesseract", font=("Segoe UI", 10),
                      bg=COLORS["bg_panel"], fg="white",
                      activebackground=COLORS["accent"], relief=tk.FLAT, padx=10, pady=8,
                      command=run_setup).pack(side=tk.LEFT, padx=5)

        tk.Button(btn_frame, text="Đóng", font=("Segoe UI", 11),
                  bg=COLORS["border"], fg="white",
                  activebackground=COLORS["text_secondary"], relief=tk.FLAT, padx=20, pady=8,
